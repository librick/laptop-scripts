...
sudo -u $username flatpak -y --user remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo
readarray -t fps_user < $plaintext_dir/flatpaks-user.txt
for fp in "${fps_user[@]}"; do
    sudo -u $username flatpak -y --user install flathub $fp
done
